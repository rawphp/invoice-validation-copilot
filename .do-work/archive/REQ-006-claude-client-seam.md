...
**Terminal state:**
**Parent:** REQ-003
**Closure proof:** checkpoint_log:passed commit:525e914
...
- [x] A `ClaudeClient` interface declares a method to send a message (with optional document/image blocks and optional tool schema) and return content + token usage.
- [x] An Anthropic implementation targets model id `claude-opus-4-8` and reads the key from `config('services.anthropic.key')`.
- [x] The interface is bound in the container so it can be swapped for a fake in tests.
- [x] A unit test binds a fake `ClaudeClient` and asserts the binding resolves to the fake.
...
- app/Services/Claude/ClaudeClient.php — Claude seam interface (message with documents/tool/tool_choice → ClaudeResponse)
- app/Services/Claude/AnthropicClaudeClient.php — real Anthropic Messages API impl (claude-opus-4-8, base64 image/pdf blocks, forced tool output)
- app/Services/Claude/ClaudeResponse.php — typed result: content + input/output tokens + text()/toolInput()/totalTokens()
- app/Services/Claude/DocumentBlock.php — base64 image/pdf content-block value object
- app/Providers/ClaudeServiceProvider.php — binds ClaudeClient → AnthropicClaudeClient (registered in bootstrap/providers.php)
- config/services.php — anthropic key/model from ANTHROPIC_API_KEY / ANTHROPIC_MODEL
- tests/Unit/Claude/ClaudeClientTest.php — binds fake into container, asserts resolution; no network
