...
**Terminal state:**
**Parent:**
**Closure proof:** checkpoint_log:passed commit:2c24e0b
...
- [x] `tailwind.config.ts` defines the Precision Ledger palette (surface, primary Deep Slate, secondary Indigo, error, success/emerald, amber) and font families (Inter, JetBrains Mono) from DESIGN.md.
- [x] Inter and JetBrains Mono are loaded (self-hosted or CDN) and applied via the theme.
- [x] `AppLayout.vue` renders a Precision Ledger styled shell (surface background `#f7f9fb`, header) and type-checks under `lang="ts"`.
- [x] A sample element using a token utility (e.g. `bg-primary`, `font-mono`) renders with the correct color/font in the browser.
...
- resources/css/app.css — Precision Ledger @theme tokens (palette, Inter+JetBrains Mono, radius/spacing), single source of truth
- tailwind.config.ts — Tailwind v4 content-source config; tokens live in CSS @theme (no duplicated hex)
- resources/js/Layouts/AppLayout.vue — Precision Ledger shell (surface bg, header, content slot), `<script setup lang="ts">`
- resources/js/types/design.ts — shared design TS types (StatusTone/Severity + token-backed pill class map)
- resources/js/Pages/Welcome.vue — demo page mounted in AppLayout consuming token utilities
